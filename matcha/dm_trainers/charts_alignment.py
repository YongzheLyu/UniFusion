...
from matcha.dm_scene.parallel_aligner_static import (
    ParallelAligner,
    MLPParams,
...
def align_charts_in_parallel(
    ...
    if save_charts_data:
        ...
        # Save each chart's depth and conf to separate directories
        # Use "depth" and "confidence" directory names to match organize_results_to_priors_format expectations
        depth_dir = os.path.join(charts_data_path, "depth")
        conf_dir = os.path.join(charts_data_path, "confidence")
        os.makedirs(depth_dir, exist_ok=True)
        os.makedirs(conf_dir, exist_ok=True)

        print(f"[INFO] Saving individual depths to {depth_dir}")
        print(f"[INFO] Saving individual confs to {conf_dir}")

        depths_np = charts_depths.cpu().numpy()
        confs_np = charts_confs.cpu().numpy()
        n_charts = depths_np.shape[0]

        for i_chart in range(n_charts):
            # 构建文件名，格式: cam{cam_id:02d}_cam_{cam_id:04d}_{frame_id:04d}_depth.npy
            # frame_id 使用 0000，因为 organize_results_to_priors_format 会从目录名获取真实帧号
            depth_file_name = f"cam{i_chart:02d}_cam_{i_chart:04d}_0000_depth.npy"
            conf_file_name = f"cam{i_chart:02d}_cam_{i_chart:04d}_0000_conf.npy"

            np.save(os.path.join(depth_dir, depth_file_name), depths_np[i_chart])
            np.save(os.path.join(conf_dir, conf_file_name), confs_np[i_chart])
