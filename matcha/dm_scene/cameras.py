...
class GSCamera(torch.nn.Module):
    ...
    def to(self, device):
        # Update device
        self.data_device = torch.device(device) if isinstance(device, str) else device

        # Move underlying tensors to device
        ...
        if hasattr(self, 'prcppoint'):
            self.prcppoint = self.prcppoint.to(device)
        if hasattr(self, 'gt_alpha_mask') and self.gt_alpha_mask is not None:
            self.gt_alpha_mask = self.gt_alpha_mask.to(device)
