...
class ChartsEncodingParams():
    def __init__(
        ...
        resolution_factor:float=0.8,  # TODO: Changed
...
class MLPParams():
    def __init__(
        self,
        n_deformation_layers:int=5,  # TODO: Changed
        deformation_layer_size:int=256,  # TODO: Changed
...
class ParallelAligner(torch.nn.Module):
    ...
    def __init__(
        ...
        print(depths.shape[0], len(cameras))
        if isinstance(cameras, list):
            print(len(cameras[0]))
            assert depths.shape[0]/len(cameras) == len(cameras[0])
        else:
        ...
        if n_timestamps is None:
            ...
            n_charts = int(n_pm / n_timestamps)
        #print("n_pm:", n_pm, "n_timestamps:", n_timestamps)
        ...
        #print("stage 1: no cameras loading params")
        #等待用户输入一个数字才继续执行
        #input_num = input("输入任意数字并回车以继续: ")
        self.cameras = cameras
        print("device:", device)
        # Create 3D points and rays (keep on device for efficiency)
        
        #try to accelerate the backprojection
        _verts = torch.zeros(n_pm, pm_h, pm_w, 3, device="cpu")
        for i_depth in range(n_pm):
            print("backprojecting depth: ", i_depth)
            _verts[i_depth] = cameras.backproject_depth(cam_idx=i_depth, depth=depths[i_depth].cuda()).view(pm_h, pm_w, 3)
        
        cameras = cameras.to("cpu")
        #accelerate
        self._verts = torch.nn.Parameter(_verts, requires_grad=False)
        self._depths = torch.nn.Parameter(depths.to(device), requires_grad=False)
        self._rays = (_verts - cameras.p3d_cameras.get_camera_center().view(n_pm, 1, 1, 3).to(device)).view(n_pm, -1 , 3)  # (n_charts, n_verts_per_chart, 3)
        self._deformed_verts = torch.nn.Parameter(_verts.clone(), requires_grad=False)
        print("finish verts depth initialization")
        #print("stage 3: loaded verts")
         #等待用户输入一个数字才继续执行
        #input_num = input("输入任意数字并回车以继续: ")
        # Create chart encoding (keep on device for efficiency)
        ...
        for _i_depth in range(n_pm):
            ...
            # Save depth map as heatmap
            depth_save_dir = "depth_maps"
            os.makedirs(depth_save_dir, exist_ok=True)

            # Reshape depth_i back to 2D image
            #depth_map = depth_i.reshape(pm_h, pm_w).detach().numpy()

            # Create heatmap with jet colormap
            # plt.figure(figsize=(10, 6))
            # plt.imshow(depth_map, cmap='jet', aspect='auto')
            # plt.colorbar()
            # plt.title(f'Chart {_i_depth} - Depth Map')
            # plt.axis('off')

            # # Save the heatmap
            # save_path = os.path.join(depth_save_dir, f'chart_{_i_depth:03d}_depth.png')
            # plt.savefig(save_path, bbox_inches='tight', dpi=150)
            # plt.close()

            # print(f"Saved depth heatmap: {save_path}")
            ...
            for i_quantile in range(n_quantiles_to_use_for_learnable_distance_encoding):
                _quantiles_i[i_quantile] = torch.quantile(depth_i.cuda(), i_quantile / n_quantiles_to_use_for_learnable_distance_encoding).item()
        ...
        self.depth_coords = torch.nn.Parameter(depth_coords, requires_grad=False)  # (n_charts, n_verts_per_chart)

        # Create depth encodings (keep on device)
        ...
        if use_learnable_depth_encoding and learnable_depth_encoding_mode == 'adaln':
            self.adaln = AdaLN(dim=depth_encoding_params.encoding_dim).to("cuda")
    ...
    def loss(self, reference_depths, pred_depths, masks=None):
        ...
        if self.use_learnable_confidence:
            confidence = self.confidence  # Already on the correct device
