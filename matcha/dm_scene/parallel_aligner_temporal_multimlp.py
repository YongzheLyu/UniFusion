...
from matcha.dm_deformation.multi_mlp_temporal import DeformationMultiMLPTemporal
from matcha.dm_deformation.multi_mlp_temporal_resfield import DeformationMultiMLPResField
from matcha.dm_deformation.temporal_encoding import TemporalEncoding
...
class ParallelAlignerTemporal(ParallelAligner):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.temporal_encoding_params = temporal_encoding_params
        self.resfield_rank = kwargs.get('resfield_rank', 8)
        ...
        self.temporal_encoding = TemporalEncoding(
            n_timestamps=n_timestamps,
            encoding_dim=temporal_encoding_params.encoding_dim,
            encoding_type=temporal_encoding_params.encoding_type,
            #encoding_type='positional',
            max_freq=temporal_encoding_params.max_freq,
            initialization_range=temporal_encoding_params.initialization_range,
        ).to(self.device)
        print("finish init")
        #a = input("pause")
    def _create_temporal_mlp(self, mlp_params, charts_encoding_params, depth_encoding_params):
        ...
        Override the MLP created by parent class to use DeformationMultiMLPTemporal.
        """
        ...
        # Use temporal MLP with n_heads = n_charts_per_timestamp
        # This means the same view across different timestamps share MLP parameters
        _additional_input_dim = temporal_encoding_params.encoding_dim
        ...
        self.deformation = DeformationMultiMLPResField(
            ...
            output_points=False,
            use_cam_encoding=True,
            resfield_layers=None,
            resfield_rank=self.resfield_rank,
            resfield_capacity=self.n_timestamps,
    ...
    def compute_verts_for_charts(self, chart_indices, profile=False):
        ...
        timestamps_in_batch = torch.unique(chart_indices // self.n_charts_per_timestamp)

        if len(timestamps_in_batch) == 1:
            # 只有一个时间戳，直接处理
            pts_uv_subset = self._pts_uv[chart_indices].cuda()  # (n_charts, H*W, 2)
            #print(f"pts_uv_subset.shape: {pts_uv_subset.shape}")
            if profile:
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
            ...
            for t in timestamps_in_batch:
                t = t.item()
                # Find charts belonging to this timestamp
                timestamp_chart_mask = (chart_indices // self.n_charts_per_timestamp) == t
                timestamp_chart_indices = chart_indices[timestamp_chart_mask]

                if len(timestamp_chart_indices) == 0:
                    continue

                # Extract data for this timestamp and get encodings
                pts_uv_t = self._pts_uv[timestamp_chart_indices].cuda()  # (n_views_in_t, H*W, 2)
                if profile:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    encoding_start = time.time()
                ...
                encodings[timestamp_chart_mask.cuda()] = encodings_t_.cuda()
            if profile:
        ...
        if self.weight_encodings_with_confidence:
            ...
            # Ensure same device without unnecessary .cuda() call
        ...
        if self.use_learnable_depth_encoding:
            if len(timestamps_in_batch) == 1:
                # 只有一个时间戳，直接处理
                depth_coords_subset = self.depth_coords[chart_indices].cuda()
                if profile:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                ...
                for t in timestamps_in_batch:
                    ...
                    depth_coords_t = self.depth_coords[timestamp_chart_indices].cuda()
                    if profile:
                        if torch.cuda.is_available():
                            torch.cuda.synchronize()
                        depth_encoding_start = time.time()
                    ...
                    print("depth encoding device: ", depth_encodings_t.device)
        ...
        # 3. Get temporal features (直接在GPU上计算)
        if profile:
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            step_start = time.time()
        
        timestamp_indices_subset = self.timestamp_indices[chart_indices]  # 已在GPU上
        time_features = self.temporal_encoding(timestamp_indices_subset)  # (n_charts, time_dim)
        n_verts_per_chart = encodings.shape[1]  # H*W
        time_features = time_features[:, None, :].expand(-1, n_verts_per_chart, -1)

        if profile:
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            step_times['3_temporal_encoding'] = time.time() - step_start
        ...
        # Find unique timestamps in chart_indices
        if profile:
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            step_start = time.time()
        ...
        for t in timestamps_in_batch:
            ...
            # Get additional_input components (depth encodings + camera parameters)
            #additional_input_t = None
            # (1) Depth encodings in concatenate mode
            #if (self.use_learnable_depth_encoding
            #    and self.learnable_depth_encoding_mode == 'concatenate'
            #    and depth_encodings is not None):
           #     additional_input_t = depth_encodings[timestamp_chart_mask]  # (n_views_in_t, H*W, enc_dim)
            ...
            #    additional_input_t = torch.cat([additional_input_t, cam_feats_t], dim=-1)
            #else:
            #    additional_input_t = cam_feats_t

            # Temporal features for this timestamp
            timestamp_indices_t = self.timestamp_indices[timestamp_chart_indices]  # (n_views_in_t,)
            time_features_t = self.temporal_encoding(timestamp_indices_t)  # (n_views_in_t, time_dim)
            time_features_t = time_features_t[:, None, :].expand(-1, n_verts_per_chart, -1)  # (n_views_in_t, H*W, time_dim)
            # 检查temporal_encoding的参数是否有学习率（可优化的参数）
            has_lr = any(p.requires_grad for p in self.temporal_encoding.parameters())
            print(f"[Debug] temporal_encoding has learnable parameters with requires_grad=True? {has_lr}")
            if len(timestamps_in_batch) > 1:
                time_feature_list.append(time_features_t)
                encoding_list.append(encodings_t)
            
            # Forward through MLP using pre-computed encodings
            if profile:
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                deformation_start = time.time()
            #print("encodings_t.device: ", encodings_t.device)
            #print("time_features_t.device: ", time_features_t.device)
            
            deformations_t = self.deformation(
                encodings_t,
                time_features=time_features_t.cuda(),
                cam_params=cam_feats_t.cuda(),
        ...
        # if len(encoding_list) > 1 and len(time_feature_list) > 1:
        #     enc0 = encoding_list[0]
        #     enc1 = encoding_list[1]
        #     tf0 = time_feature_list[0]
        #     tf1 = time_feature_list[1]
        #     # flatten to same shape for comparison if necessary
        #     enc0_flat = enc0.reshape(-1)
        #     enc1_flat = enc1.reshape(-1)
        #     tf0_flat = tf0.reshape(-1)
        #     tf1_flat = tf1.reshape(-1)
        #     enc_l1 = (enc0_flat - enc1_flat).abs().mean()
        #     tf_l1 = (tf0_flat - tf1_flat).abs().mean()
        #     print(f"enc_l1: {enc_l1}, tf_l1: {tf_l1}")
        ...
        rays_subset = self._rays[chart_indices].to("cuda")
        predict_deformations_along_rays = deformations.shape[-1] == 1
        #print("predict_deformations_along_rays: ", predict_deformations_along_rays)
        #print("deformations.shape: ", deformations.shape)
        no_residual = False
        # 如果deformations最后一维是3维，直接使用deformations作为verts
        print("mean offset:", deformations.abs().mean())
        if no_residual:
            ...
            verts_subset = self._verts[chart_indices].to("cuda")  # (n_charts, H*W, 3)
    ...
    def verts_deformations(self):
        ...
        for t in range(self.n_timestamps):
            ...
            additional_input = None
            if self.use_learnable_depth_encoding:
                depth_coords_t = self.depth_coords[chart_indices].cuda()
            ...
            # 3b. Add raw camera parameters (per-chart, broadcast to all pixels)
            cam_feats_t = self.camera_params[chart_indices].to(device)                  # (N, 9)
            cam_feats_t = cam_feats_t[:, None, :].expand(-1, encodings.shape[1], -1)    # (N, H*W, 9)
            if additional_input is not None:
                additional_input = torch.cat([additional_input, cam_feats_t], dim=-1)
            else:
                additional_input = cam_feats_t

            # 4. Get temporal features for this timestamp
            timestamp_indices_t = self.timestamp_indices[chart_indices]  # (N,)
            time_features = self.temporal_encoding(timestamp_indices_t)  # (N, time_dim)

            # Expand to match all vertices
            n_verts_per_chart = encodings.shape[1]  # H*W
            time_features = time_features[:, None, :].expand(-1, n_verts_per_chart, -1)  # (N, H*W, time_dim)

            # 5. Forward through temporal MLP
            deformations_t = self.deformation(
                encodings,
    ...
    def reset_encodings(self):
        """Reset encodings for temporal version.

        Override parent method to also reset temporal encoding.
        """
        # Call parent's reset_encodings to handle charts_encoding and depth_encoding
        super().reset_encodings()

        # Reset temporal encoding
        self.temporal_encoding = TemporalEncoding(
            n_timestamps=self.n_timestamps,
            encoding_dim=self.temporal_encoding_params.encoding_dim,
            encoding_type=self.temporal_encoding_params.encoding_type,
            max_freq=self.temporal_encoding_params.max_freq,
            initialization_range=self.temporal_encoding_params.initialization_range,
        ).to(self.device)
    ...
    def reset_mlp(self, std=None):
        ...
        Override parent method to use DeformationMultiMLPTemporal instead of DeformationMultiMLP.
        """
        if self.use_meta_mlp or self.use_lora_mlp:
            ...
            # additional_input: depth encodings (concatenate mode) + raw camera params (9D)
            ...
            self.deformation = DeformationMultiMLPTemporal(
                ...
                time_feature_dim=self.temporal_encoding_params.encoding_dim,
                #additional_input_dim=_additional_input_dim,
                data_input_range_min=-scene_radius,
                data_input_range_max=scene_radius,
                mlp_input_range_min=-1.,
        ...
        for i_iter in range(n_iterations):
            ...
            if use_gradient_loss:
                ...
                if gradient_masks is not None:
                    ...
                    if enable_memory_efficient and batch_timestamps < self.n_timestamps:
                        ...
                        depths_to_compare = self._depths[chart_indices]
                    else:
                        gradient_masks_batch = gradient_masks
                        depths_to_compare = self._depths
                    grad_loss = gradient_loss_weight * (gradient_masks_batch * (
                        img_grad(_deformed_depths_batch) - img_grad(depths_to_compare)
                    )).abs().mean()
                else:
                    depths_to_compare = self._depths[chart_indices] if (enable_memory_efficient and batch_timestamps < self.n_timestamps) else self._depths
            ...
            if use_hessian_loss:
                ...
                if gradient_masks is not None:
                    ...
                    hess_loss = hessian_loss_weight * (gradient_masks_batch * (
                        img_hessian(_deformed_depths_batch) - img_hessian(depths_to_compare)
                    )).abs().mean()
                else:
                    depths_to_compare = self._depths[chart_indices] if (enable_memory_efficient and batch_timestamps < self.n_timestamps) else self._depths
            ...
            if use_normal_loss:
                ...
                if enable_memory_efficient and batch_timestamps < self.n_timestamps:
                    ...
                    # Ensure that the transforms are on the same device as the input depths
                    depths_batch = self._depths[chart_indices].to("cuda")
                    world_view_transforms_batch = world_view_transforms_batch.to(depths_batch.device)
                    full_proj_transforms_batch = full_proj_transforms_batch.to(depths_batch.device)
