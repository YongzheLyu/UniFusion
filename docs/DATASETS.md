...
Each sequence starts with four synchronized camera directories under
`<DATA_ROOT>/<sequence>/grouped_by_cams`. The `prepare` stage standardizes these
images under `<DATA_ROOT>/<sequence>/dataset/renamed_images` and constructs
numbered `frame_XXXXX` directories under `frames_output`.
...
python scripts/reproduce_exorecon.py \
  --data-root "$DATA_ROOT" \
  --stages prepare preprocess align organize finalize \
  --dry-run
```
