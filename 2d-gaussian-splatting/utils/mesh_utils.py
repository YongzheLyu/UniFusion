...
class GaussianExtractor(object):
    ...
    @torch.no_grad()
    def export_image(self, path, depth_normalize=True):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            depth_raw = self.depthmaps[idx][0].cpu().numpy()

            # Save raw depth as TIFF (float32) - preserves full precision
            save_img_f32(depth_raw, os.path.join(vis_path, 'depth_raw_{0:05d}'.format(idx) + ".tiff"))

            # Save raw depth as NPY for easy loading
            np.save(os.path.join(depth_path, '{0:05d}'.format(idx) + ".npy"), depth_raw)

            if depth_normalize:
                # Normalize depth for visualization (handle inf and nan values)
                depth_vis = np.copy(depth_raw)
                valid_mask = np.isfinite(depth_vis) & (depth_vis > 0)

                if np.any(valid_mask):
                    depth_min = np.min(depth_vis[valid_mask])
                    depth_max = np.percentile(depth_vis[valid_mask], 95)  # Use 95th percentile to avoid outliers

                    # Normalize to [0, 1] range for visualization
                    depth_vis[valid_mask] = (depth_vis[valid_mask] - depth_min) / (depth_max - depth_min)
                    depth_vis[~valid_mask] = 1.0  # Set invalid values to white (far)
                    depth_vis = np.clip(depth_vis, 0.0, 1.0)
                else:
                    depth_vis = np.ones_like(depth_vis)  # All white if no valid depths

                # Save normalized depth as PNG for easy viewing
                save_img_u8((depth_vis * 255).astype(np.uint8), os.path.join(vis_path, 'depth_vis_{0:05d}'.format(idx) + ".png"))

                # Also save as grayscale for better depth perception
                depth_gray = (depth_vis * 255).astype(np.uint8)
                Image.fromarray(depth_gray, mode='L').save(os.path.join(vis_path, 'depth_gray_{0:05d}'.format(idx) + ".png"))
