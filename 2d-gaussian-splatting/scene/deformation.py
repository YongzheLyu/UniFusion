...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        with torch.no_grad():
            ...
            # Rotation deformation last layer
            self.rotations_deform[-1].weight.zero_()
            self.rotations_deform[-1].bias.zero_()
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        hidden = self.query_time(rays_pts_emb.cuda(), scales_emb.cuda(), rotations_emb.cuda(), time_feature, time_emb.cuda())
        if self.args.static_mlp:
        ...
        if self.args.no_dr :
            ...
            dr = self.rotations_deform(hidden).cuda()
