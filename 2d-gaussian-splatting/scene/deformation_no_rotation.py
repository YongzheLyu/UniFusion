...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        with torch.no_grad():
            ...
            # Removed rotation deformation initialization
            # Opacity deformation last layer
            self.opacity_deform[-1].weight.zero_()
            self.opacity_deform[-1].bias.zero_()
    ...
    def query_time(self, rays_pts_emb, scales_emb, time_feature, time_emb):
        # Removed rotations_emb parameter
        if self.no_grid:
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        # Removed rotations_emb from query_time call
        ...
        # Removed rotation deformation logic - rotations pass through unchanged
        rotations = rotations_emb[:,:4]

        if self.args.no_do :
            opacity = opacity_emb[:,:1]
        else:
            do = self.opacity_deform(hidden).cuda()
...
class deform_network(nn.Module):
    ...
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, shs=None, times_sel=None):
        ...
        # Removed rotations positional encoding
