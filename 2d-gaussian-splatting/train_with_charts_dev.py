...
def get_dataset_info(scene):
    train_cameras = scene.getTrainCameras()
    test_cameras = scene.getTestCameras()
    max_frame = 0
    cam_num = 0
    for train_cam in train_cameras:
        frame_id = int(train_cam.image_name.split('_')[2])
        cam_id = int(train_cam.image_name.split('_')[1])
        if frame_id > max_frame:
            max_frame = frame_id
        if cam_id > cam_num:
            cam_num = cam_id
    for test_cam in test_cameras:
        frame_id = int(test_cam.image_name.split('_')[2])
        cam_id = int(test_cam.image_name.split('_')[1])
        if frame_id > max_frame:
            max_frame = frame_id
        if cam_id > cam_num:
            cam_num = cam_id
   # train_frames = [cam.frame_id for cam in train_cameras]
    #train_cams = [cam.cam_id for cam in train_cameras]
    #cam_num = max(train_cams)
    #max_frame = max(train_frames)
    return cam_num, max_frame
def load_preprocessed_priors_from_dir(preprocessed_priors_dir, cameras, device="cpu", frames=10):
    ...
    for i, cam in enumerate(cameras):
        ...
        if len(frame_match) >= 2:
            try:
                frame_id = int(frame_match[2].split('.')[0]) # 提取00001 -> 1
        ...
        # 相机ID映射: 相机索引0-9 -> cam_id=1, 10-19 -> cam_id=2, 等等
        #cam_id = (i // frames) 
        frame_id = frame_id - 1
        #11.4 debug cam_id -1
        # 构建文件名
...
def scene_reconstruction(
    ...
    if stage == "coarse":
        ...
        print("[INFO] Coarse stage: Using only first frame cameras")
    else:
    ...
    for iteration in range(first_iter, final_iter + 1001 if stage == "fine" and opt.freeze_gaussians_in_fine else final_iter + 1):
        iter_start.record()

        # Freeze/unfreeze Gaussian parameters in fine stage
        if stage == "fine" and opt.freeze_gaussians_in_fine:
            if iteration <= 1000:
                # Freeze Gaussian parameters (only update deformation network)
                if iteration == 1:
                    print("[INFO] Fine stage: Freezing Gaussian parameters for first 1000 iterations (only updating deformation network)")
                for param_group in gaussians.optimizer.param_groups:
                    if param_group["name"] in ["xyz", "f_dc", "f_rest", "opacity", "scaling", "rotation"]:
                        param_group["lr"] = 0.0
            elif iteration == 1001:
                # Unfreeze Gaussian parameters after 1000 iterations
                print("[INFO] Fine stage: Unfreezing Gaussian parameters after 1000 iterations")
                # Restore learning rates from training_setup
                for param_group in gaussians.optimizer.param_groups:
                    if param_group["name"] == "xyz":
                        param_group['lr'] = opt.position_lr_init * gaussians.spatial_lr_scale
                    elif param_group["name"] == "f_dc":
                        param_group['lr'] = opt.feature_lr
                    elif param_group["name"] == "f_rest":
                        param_group['lr'] = opt.feature_lr / 20.0
                ...
                gaussians.update_learning_rate(iteration)  # Apply scheduling
        ...
        # chart / dense regularization (与原逻辑一致)
        ...
        if iteration % use_chart_view_every_n_iter == 0:
            ...
            # Old style depth prior (与你原代码一致)
            # depth_prior_loss = lambda_prior_depth * (
            #     confidence_weighting * charts_confs[viewpoint_idx] * torch.log(1. + charts_scale_factor * (charts_depths[viewpoint_idx] - surf_depth).abs())
            # ).mean()
            #print("[INFO] processing depth in shape of", charts_depths[viewpoint_idx].shape)
            # chart_depth_upsampled = F.interpolate(
            #     charts_depths[viewpoint_idx].unsqueeze(0).unsqueeze(0),
            #     size=surf_depth.shape[-2:],
            #     mode='bilinear',  # 或 'nearest'，视需求而定
            #     align_corners=False
            # )
            # #print("[INFO] processing normal in shape of", charts_normals[viewpoint_idx].shape)
            # chart_normal_upsampled = F.interpolate(
            #     charts_normals[viewpoint_idx].unsqueeze(0),
            #     size=rend_normal.shape[1:],
            #     mode='bilinear',  # 或 'nearest'，视需求而定
            #     align_corners=False
            # )
            # #print("[INFO] processing conf in shape of", charts_confs[viewpoint_idx].shape)
            # chart_conf_upsampled = F.interpolate(
            #     charts_confs[viewpoint_idx].unsqueeze(0).unsqueeze(0),
            #     size=surf_depth.shape[1:],
            #     mode='bilinear',  # 或 'nearest'，视需求而定
            #     align_corners=False
            # )
            # #print("[INFO] processing curv in shape of", charts_curvs[viewpoint_idx].shape)
            # chart_curv_upsampled = F.interpolate(
            #     charts_curvs[viewpoint_idx].unsqueeze(0),
            #     size=rend_curvature.shape[1:],
            #     mode='bilinear',  # 或 'nearest'，视需求而定
            #     align_corners=False
            # )
            
            #print(charts_depths[viewpoint_idx].shape, surf_depth.shape)
            
           # print(charts_depths_up[viewpoint_idx].shape, surf_depth.shape)
            #print(f"surf depth min {surf_depth.min()} max {surf_depth.max()}")
            #print(f"prior depth min {prior_depth.min()} max {prior_depth.max()}")
            depth_prior_loss = lambda_prior_depth * (
            ...
            # dense supervision branch (与你原代码一致)
        ...
        with torch.no_grad():
            ...
            if iteration % 500 == 0:
                with torch.no_grad():
                    print(f"\n[ITER {iteration}] Evaluating on test cameras...")
                    test_cameras = scene.getTestCameras()
                    if len(test_cameras) > 0:
                        psnr_test = 0.0
                        l1_test = 0.0
            ...
            should_skip_densification = (stage == "fine" and opt.freeze_gaussians_in_fine and iteration <= 1000)
            ...
            if iteration < opt.densify_until_iter and not should_skip_densification:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 :
                    ...
                    if hasattr(gaussians, "densify_and_prune"):
                        gaussians.densify_and_prune(densify_threshold, opt.opacity_cull, scene.cameras_extent, size_threshold, pts_num)
                    else:
...
def training(
    ...
    dense_regul, coarse_iterations, preprocessed_priors_dir=None, cam_num=8,
    frame_interval=30, enable_group_training=False
):
    """
    重构后的 training：负责初始化 gaussians、charts、dense 数据等，
    然后依序调用 scene_reconstruction 两次（coarse -> fine）。
    支持分组训练：将cameras按帧间隔分组，每组使用独立的gaussians训练
    """
    ...
    # 检查是否启用分组训练
    if enable_group_training:
        print("=" * 80)
        print("[INFO] Group-based training enabled!")
        print(f"[INFO] Frame interval: {frame_interval} frames per group")

        # 获取所有训练相机
        all_train_cameras = scene.getTrainCameras()
        total_cameras = len(all_train_cameras)
        #train_frames = [cam.frame_id for cam in all_train_cameras]
        #train_cams = [cam.cam_id for cam in all_train_cameras]
        cam_num, max_frame =  get_dataset_info(scene)
        #cam_num = max(train_cams)
        print(f'[INFO] Camera number: {cam_num}')
        #max_frame = max(train_frames)
        # 计算需要多少组
        num_groups = (max_frame + frame_interval - 1) // frame_interval
        print(f"[INFO] Total cameras: {total_cameras}, Number of groups: {num_groups}")

        # 存储每组的gaussians模型用于后续初始化
        prev_group_gaussians = None

        for group_idx in range(num_groups):
            start_idx = group_idx * frame_interval * cam_num
            end_idx = min((group_idx + 1) * frame_interval * cam_num, total_cameras)

            print("=" * 80)
            print(f"[INFO] Training Group {group_idx + 1}/{num_groups}")
            print(f"[INFO] Frame range: {start_idx} to {end_idx - 1} ({end_idx - start_idx} frames)")
            print("=" * 80)

            # 为当前组创建新的gaussians模型
            group_gaussians = GaussianModel(dataset.sh_degree, dataset)

            # 如果不是第一组,使用前一组的最后一帧点云进行初始化
            if group_idx > 0 and prev_group_gaussians is not None:
                print(f"[INFO] Initializing Group {group_idx + 1} from Group {group_idx}'s static point cloud")
                ...
                with torch.no_grad():
                    ...
                    print(f"[INFO] Inherited {_means.shape[0]} points from previous group")
                    group_gaussians.create_from_parameters(_means, _scales, _quaternions, _colors,
                                                          prev_group_gaussians.spatial_lr_scale)
            else:
                # 第一组: 使用原始的charts数据初始化
                print(f"[INFO] Initializing Group 1 from charts data")

                # 筛选当前组的相机
                group_cameras = all_train_cameras[start_idx:end_idx]
                ...
                frames = len(group_cameras) // cam_num
                # 获取当前组的图像
                _images = [
                    torch.nn.functional.interpolate(cam.original_image[None].cuda(),
                                                   (h_charts, w_charts),
                                                   mode="bilinear", antialias=True)[0].permute(1, 2, 0)
                    for cam in group_cameras[::frames]
                ]
            ...
            # 创建当前组的scene (使用筛选后的相机)
            # 临时修改dataset的cameras来只包含当前组
            original_train_cameras = scene.train_cameras
            scene.train_cameras[1.0] = all_train_cameras[start_idx:end_idx]

            # 为当前组创建输出目录
            group_model_path = os.path.join(dataset.model_path, f'group_{group_idx + 1}')
            os.makedirs(group_model_path, exist_ok=True)
            original_model_path = dataset.model_path
            dataset.model_path = group_model_path

            print(f"[INFO] Group {group_idx + 1} model will be saved to: {group_model_path}")

            # 训练当前组 (coarse阶段)
            if coarse_iterations and coarse_iterations > 0:
                print(f"[INFO] Group {group_idx + 1}: Starting COARSE stage")
            ...
            # 训练当前组 (fine阶段)
            print(f"[INFO] Group {group_idx + 1}: Starting FINE stage")
            scene_reconstruction(
                dataset=dataset,
                opt=opt,
                pipe=pipe,
            ...
            # 保存当前组的模型
            print(f"[INFO] Saving Group {group_idx + 1} model")
            scene.save(opt.iterations)

            # 保存当前组的gaussians作为下一组的初始化
            prev_group_gaussians = group_gaussians

            # 恢复原始路径
            dataset.model_path = original_model_path
            scene.train_cameras = original_train_cameras

            # 清理内存
            torch.cuda.empty_cache()
            gc.collect()

            print(f"[INFO] Group {group_idx + 1} training completed!")
            print("=" * 80)

        print("[INFO] All groups training completed!")
        return  # 分组训练完成后直接返回
    ...
    print('='*50)
    train_cameras = scene.getTrainCameras()
    frames = len(train_cameras) // cam_num
    _images = [
    ...
    print(len(_images))
    ply_path = f'{dataset.source_path}/points3D_multipleview.ply'
    ...
    if True:
        ...
        gaussian_params = get_gaussian_parameters_from_charts_data(
            charts_data=filtered_charts_data,  # Use filtered data
            images=_images,
            conf_th=-1.,  # TODO
            ratio_th=5.,
            normal_scale=1e-10,
    ...
    if tb_writer:
        tb_writer.add_text('stage_transition', "Starting FINE stage", coarse_iterations if coarse_iterations else 0)
        tb_writer.add_scalar('stage/current', 1, coarse_iterations if coarse_iterations else 0)  # 1 for fine
    scene_reconstruction(
        dataset=dataset,
        opt=opt,
        pipe=pipe,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cam_num', type=int, default=8)
    parser.add_argument('--frame_interval', type=int, default=30, help='Number of frames per group for incremental training')
    parser.add_argument('--enable_group_training', action='store_true', default=False, help='Enable group-based training with frame intervals')
    ...
    training(
        ...
        args.dense_regul, args.coarse_iterations, args.preprocessed_priors_dir, args.cam_num,
        args.frame_interval, args.enable_group_training
    )
