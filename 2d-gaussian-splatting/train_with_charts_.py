...
def load_preprocessed_priors_from_dir(preprocessed_priors_dir, cameras, device="cpu", frames=10):
    ...
    if metadata_path.exists():
        ...
        # 优先从 {priors_dir}/charts/frame_00001/charts_data.npz 读取
        charts_frame_path = priors_dir / "charts" / "frame_00001" / "charts_data.npz"
        if charts_frame_path.exists():
            charts_data = np.load(charts_frame_path)
            scale_factor = charts_data.get('scale_factor', 1.0)
            print(f"[INFO] 从 {charts_frame_path} 加载 scale_factor")
        else:
            # 回退到 priors_dir/charts_data.npz
            prior_path = priors_dir / "charts_data.npz"
            if prior_path.exists():
                charts_data = np.load(prior_path)
                scale_factor = charts_data.get('scale_factor', 1.0)
                print(f"[INFO] 从 {prior_path} 加载 scale_factor")
            else:
                print(f"[WARNING] 未找到charts数据文件: {charts_frame_path} 或 {prior_path}")
                scale_factor = 1.0
...
def scene_reconstruction(
    ...
    preprocessed_priors_dir: str = None
):
    """
    ...
    if preprocessed_priors_dir is None:
    ...
    for iteration in range(first_iter, final_iter + warmup_iter + 1 if stage == "fine" and opt.freeze_gaussians_in_fine else final_iter + 1):
        ...
        loss = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)))*2
        # 假设 gaussians 是你的 GaussianModel 实例
        # 1. 获取物理缩放值 exp(s)，形状通常为 [N, 3]
        scales = gaussians.get_scaling 

        # 2. 计算 L2 范数（即公式中的 ||exp(s_i)||_2）
        # 这里我们对每一个高斯球的 (sx, sy, sz) 计算向量长度
        scale_norms = torch.norm(scales, p=2, dim=-1)

        # 3. 计算所有高斯球的平均或总和作为 Loss
        # 通常建议用 mean，这样 loss 的量级受点数（N）波动的影响较小
        loss_scale_abs = scale_norms.mean()
        #loss += 0.01 * loss_scale_abs
        if iteration % 100 == 0 :
            print("scale reg", loss_scale_abs)
        ...
        if iteration % use_chart_view_every_n_iter == 0:
            ...
            # delta = 0.1  # 这是一个可以调优的超参数，建议从 0.1 或 0.2 开始
            # huber_val = F.huber_loss(surf_depth * charts_scale_factor, 
            #                         prior_depth * charts_scale_factor, 
            #                         reduction='none', 
            #                         delta=delta)

            # # 3. 结合你的置信度权重
            # depth_prior_loss = lambda_prior_depth * (confidence_weighting * prior_conf * huber_val).mean()
...
def training(
    ...
    if dataset.preprocessed_priors_dir:
        charts_data_path = os.path.join(dataset.preprocessed_priors_dir, 'charts', 'frame_00001', 'charts_data.npz')
        if not os.path.exists(charts_data_path):
            charts_data_path = f'{dataset.source_path}/charts_data.npz'
            print(f"[INFO] 未找到预处理priors中的charts数据，使用默认路径: {charts_data_path}")
        else:
            print(f"[INFO] 从预处理priors加载charts数据: {charts_data_path}")
    else:
        charts_data_path = f'{dataset.source_path}/charts_data.npz'
    charts_data = load_charts_data(charts_data_path)
    ...
    if coarse_iterations and coarse_iterations > 0:
        ...
        scene_reconstruction(
            ...
            tb_writer=tb_writer,
            frames=frames,
            preprocessed_priors_dir=dataset.preprocessed_priors_dir
        )
    ...
    # Set AABB for deformation network based on current Gaussian xyz range
    with torch.no_grad():
        print("[INFO] Computing AABB for deformation network...")
        xyz = gaussians.get_xyz  # (N, 3)
        xyz_min = xyz.min(dim=0)[0]  # (3,)
        xyz_max = xyz.max(dim=0)[0]  # (3,)
        print(f"[INFO] AABB computed: xyz_min={xyz_min.tolist()}, xyz_max={xyz_max.tolist()}")
        gaussians._deformation.deformation_net.set_aabb(xyz_max.max() if xyz_max.max() < 20 else 20,  xyz_min.min() if xyz_min.min() > -20 else -20)
        print("[INFO] Deformation network AABB set successfully")
