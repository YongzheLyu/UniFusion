...
def evaluate_tsdf(points, views, gaussians:GaussianModel, pipeline, background, return_color=False, 
    ...
    with torch.no_grad():
        for _, view in enumerate(tqdm(views, desc="Rendering progress")):
            render_pkg = render(view, gaussians, pipeline, background, stage="fine")
...
def marching_tetrahedra_with_binary_search(
    ...
    if timestamp is not None:
        ...
        time = torch.full((xyz.shape[0], 1), timestamp, dtype=xyz.dtype, device=xyz.device)
        deformed = gaussians._deformation(
            xyz, gaussians.get_scaling, gaussians._rotation,
            gaussians._opacity, gaussians.get_features, time,
        )
        tetra_kwargs = {"xyz": deformed[0], "scaling": deformed[1], "rotation": deformed[2]}
    points, points_scale = gaussians.get_tetra_points(
        downsample_ratio=downsample_ratio,
        gaussian_flatness=gaussian_flatness * spatial_extent,
        **tetra_kwargs,
    )
    # load cell if exists
...
def extract_mesh(
    ...
    with torch.no_grad():
        ...
        if dense_data_path is not None:
            ...
            dense_gaussians = GaussianModel(dataset.sh_degree, dataset)
        ...
        if timestamp is not None:
            matched_cams = [cam for cam in _cams if abs(float(cam.time) - timestamp) <= timestamp_tolerance]
            if matched_cams:
                _cams = matched_cams
            else:
                # Keep one camera per physical pose when the requested time is interpolated.
            ...
            for cam in _cams:
                cam.time = timestamp
            print(f"[INFO] Dynamic extraction at t={timestamp:.6f} using {len(_cams)} cameras.")
